...
class DataController(BaseController):
    ...
    def validate_uploaded_file(self, file: UploadFile):
        if file.content_type not in self.app_setting.FILE_ALLOWED_TYPES:
            return False, ResponseMessagesEnum.FILE_TYPE_NOT_ALLOWED.value
        ...
        if file_size > self.app_setting.FILE_MAX_SIZE * self.size_scale:
            return False, ResponseMessagesEnum.FILE_SIZE_TOO_LARGE.value

        return True, ResponseMessagesEnum.FILE_UPLOAD_SUCCESS.value

    def generate_unique_file_path(self, src_file_name: str, project_id: str):
        random_key = self.generate_random_string()
        project_path = ProjectController(app_setting=self.app_setting).get_project_path(
            project_id
        )
    ...
    async def handle_file_upload(self, project_title: str, file: UploadFile, db_client):
        ...
        # 3. Generate Path
        file_path, asset_name = self.generate_unique_file_path(
            file.filename, project_title
        )
        ...
        return (
            True,
            ResponseMessagesEnum.FILE_UPLOAD_SUCCESS.value,
            str(asset_record.id),
        )
